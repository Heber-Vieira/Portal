...
export const SaaSCard: React.FC<SaaSCardProps> = ({ saas, onDelete, onTogglePin, onEdit, onAccess, isAdmin, isDense, isDarkMode, zoomLevel = 3, hoverScale = 1.05 }) => {
    ...
    const getTitleSize = () => {
        ...
        if (zoomLevel === 2) return 'text-[8.5px]';
    ...
    const getDescriptionSize = () => {
        if (zoomLevel <= 1) return 'text-[6px]';
        if (zoomLevel === 2) return 'text-[7px]';
        if (zoomLevel === 3) return 'text-[9px]';
        if (zoomLevel === 4) return 'text-[11px]';
        return 'text-xs';
    };
            ...
            <div className={`flex items-start justify-between ${zoomLevel <= 2 ? 'mb-0.5' : 'mb-2'}`}>
                <div className="flex items-start space-x-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0 pr-2">
                        <span className={`block font-black uppercase tracking-widest text-slate-400 leading-none ${zoomLevel <= 2 ? 'text-[5px] mb-0.5' : 'text-[6px] mb-1'}`}>
                            {saas.category}
                        </span>
